...
public class HolySmite extends DamageSkill implements TargetSelectable {
	...
	public HolySmite(BaseCharacter user) {
		super("HolySmite", user, new ArrayList<logic.rank>(Arrays.asList(logic.rank.first, logic.rank.second)),
				new ArrayList<logic.rank>(Arrays.asList(logic.rank.first, logic.rank.second)), 0, 85, 0);
	}
	...
	public void playAnimation() {
		for(BaseCharacter e : targets) {
			...
			GameLogic.getCurrentStage().getBattlePane().showBattleText("CRUSADER used HOLY SMITE!");
			Audio.attack.stop();
			Audio.attack.play();
